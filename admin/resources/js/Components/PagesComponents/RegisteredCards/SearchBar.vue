<template>
    ...
    <div class="flex flex-col space-y-2">
      <div>
        <input :value="modelValue" @input="updateValue($event.target.value)" type="text"
          placeholder="Search by student id or uid"
      ...
      <div>
        <button @click="$emit('add-card')" class="w-full btn text-white btn-success shadow-lg hover:bg-[#20714c]">
          Add Card
        </button>
      </div>
    </div>
  ...
  <div class="hidden sm:block">
    <div class="flex mb-2">
      <input :value="modelValue" @input="onInput" type="text" placeholder="Search by student id or uid"
        class="w-full p-2 border border-gray-300 rounded bg-white dark:bg-gray-700 dark:text-white focus:outline-none focus:ring-2 focus:ring-blue-500" />
    ...
    <button @click="emitAddCard" class="btn text-white btn-success shadow-lg hover:bg-[#20714c] mb-2">
      Add Card
    </button>
  </div>
</template>
...
const emit = defineEmits(['update:modelValue', 'search', 'reset', 'add-card']);
