<template>
    <div ref="container" tabindex="0" class="max-w-5xl mx-auto px-4 py-6" @keydown.enter="handleEnter">
        <form ref="userForm" @submit.prevent="handleSubmit" @keydown.enter="handleEnter">
            ...
            <div v-if="form.errors.name" class="text-sm mb-2 text-red-500 dark:text-red-500">{{ form.errors.name }}
            </div>
            ...
            <div v-if="form.errors.email" class="text-sm mb-2 text-red-500 dark:text-red-500">{{ form.errors.email }}
            </div>

            <!-- Modal Actions -->
            <div class="modal-action mt-4">
                <button type="button" @click="resetForm" class="mr-4 hover:underline text-black dark:text-white">
                    Cancel
                </button>
                <button type="submit"
